 ...
 *
 * A candidate exactly `MAX_SUGGESTION_DISTANCE` edits away is not offered — only strictly
 * fewer edits count as "near". `MAX_SUGGESTION_DISTANCE` is passed straight through as the
 * `damerauLevenshtein` cutoff, so the walk still explores that far before giving up.
 */
