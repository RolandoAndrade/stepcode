...
checked each against the v2 output and printed what it could not confirm. Run it (or
`scripts/record-run.ts`, for a program no v1 test exercises) from the repo root with:
...
`register.mjs` lets plain Node resolve this codebase's extensionless imports; see its header
comment for why it exists.

- **Booleans.** v1 printed `true` / `false`; v2 prints the profile's `Verdadero` / `Falso`. The
  script rewrote those two strings before comparing. Programs touched: `equality-between-constant-and-variable`,
  `equality-between-variable-and-constant`, `greater-than-between-constant-and-variable`,
  `greater-than-between-variable-and-constant`, `greater-than-or-equal-between-constant-and-variable`,
  `greater-than-or-equal-between-variable-and-constant`, `inequality-between-constant-and-variable`,
  `inequality-between-variable-and-constant`, `less-than-between-constant-and-variable`,
  `less-than-between-variable-and-constant`, `less-than-or-equal-between-constant-and-variable`,
  `less-than-or-equal-between-variable-and-constant`, `test-false-and-true`, `test-false-or-true`,
  `test-left-greater-than-right-integer`, `test-left-greater-than-right-integer-2`,
  `test-left-less-than-right-integer`, `test-left-less-than-right-integer-2`, `test-not-false`,
  `test-not-true`, `test-right-greater-than-left-integer`, `test-right-greater-than-left-integer-2`,
  `test-right-less-than-left-integer`, `test-right-less-than-left-integer-2`, `test-true-and-false`,
  `test-true-and-true`, `test-true-or-false`, `test-true-or-true`, `test-two-different-integers`,
  `test-two-different-integers-2`, `test-two-equal-integers`, `test-two-equal-integers-2`,
  `test-two-equal-integers-3`, `test-two-equal-integers-4`, `test-two-equal-integers-5`,
  `test-two-equal-integers-6`, `test-valid-triangle`.
- **`Redondear(-1.5)`.** v1 asserted `-1` (JS `Math.round`); v2 rounds half away from zero (§5.8)
  and `test-round.run.json` records `-2`. One intentional divergence.
- **`valid-triangle-and-type`'s "invalid triangle" case.** v1's `Triangulo` writes `iguales`
  only inside the `Sino` branch, then `Escribir iguales` unconditionally after `FinSi`; the
  invalid-triangle input takes the `Si` branch, so v1 read `iguales` unassigned and printed
  nothing for it, while v2 correctly raises E4003 (§3.6, read-before-assigned) on that same
  read. `valid-triangle-and-type.run.json` records the three cases that do assign `iguales`
  (equilateral, isosceles, scalene); the invalid-triangle case is not representable as a
  `done` run and is not in the sidecar. A second, intentional divergence.
- **Computed expectations** (`(2 * 3 / 5).toString()` and the like, and `test-random`'s
  `/^0\.\d+$/`) were confirmed by hand:
  - `mul-and-div-have-the-same-priority`: `(2 * 3 / 5).toString()` is `1.2`, `(2 / 3 * 5).toString()` is `3.333333333333333` — both match the recorded output.
  - `nested-divisions`: `(2 / 3 / 5).toString()` is `0.13333333333333333` — matches.
  - `test-random`: seeded with `1`, the recorded output `0.6270739405881613\n` matches `/^0\.\d+$/`.
  - `test-nested-for-statement`, `test-nested-for-statement-with-break`, `test-nested-while-statement`,
    `test-nested-while-statement-with-break`, `test-nested-while-statement-with-continue`: each
    v1 assertion was the template literal `` `${i} * ${j} = ${i * j}` ``; the recorded output is
    the matching multiplication-table line for every `(a, b)` pair the loop actually reaches.
  - `test-true-value` (`test 1`, `test 2`): the assertions were `` `${x}${yy}${z}` `` and
    `` `${x && yy || z}` ``; for both input sets the recorded output is the boolean values that
    program's comparisons produce, concatenated and combined exactly as those expressions say.
- **Programs no v1 test made a checkable assertion against** — `test-if-statement-with-false-condition`
  and `test-missing-semicolon-at-line-2` (the v1 test ran the program but asserted nothing about
  its output) — were read by hand against their source: the first's `Si` is false so it prints
  nothing, and the recorded empty output is correct; the second computes `10 + 20` and its
  recorded `30\n` is correct. `scripts/extract-runs.ts` found a v1 test for every program in this
  directory, so `scripts/record-run.ts` — written for a program no v1 test ever exercised — was
  not needed this pass; it stays for the next program the corpus grows without one.
