...
- Test: `packages/profiles/test/schema.test.ts`
...
- [ ] **Step 2: Write the failing test `packages/profiles/test/schema.test.ts`**
...
export const completeInput = () => ({
  id: 'test',
  locale: 'es',
  keywords: { ...fill(KEYWORD_KEYS, 'kw'), case: [] },
...
import { resolveProfile } from '../src/resolve'
import { completeInput } from './schema.test'
...
describe('resolveProfile — extends', () => {
  ...
  it('a child that changes nothing equals its parent except id', () => {
    ...
    expect({ ...child, id: 'base' }).toEqual(parent)
  })
...
Expected: all 11 resolve tests pass (the "child equals parent" test passes because both carry empty lookups and the same `normalize` semantics — `toEqual` compares functions by reference, so if it fails on `normalize`, compare `{ ...child, id: 'base', normalize: undefined }` against the same projection of the parent and note the adjustment in the report). Typecheck silent.
...
describe('resolveProfile — validation', () => {
  ...
  it('detects an operator collision separately from words', () => {
    ...
    expect(() => resolveProfile(input)).toThrow(
      expect.objectContaining({ code: 'PROFILE_COLLISION', path: ['operators', 'assign'] }),
    )
  })
